...
######################################################################
#### Analysis of Rhopalosiphum padi occupancy
######################################################################

# Collectors of potato psyllids, from RawRecords data set in making_species_lists
rpadiCollectors <- readRDS("output/Rhopalosiphum_padi_collectors.rds")
# select only lists that contain collectors of potato psyllids
rpadiLists <- onlyCollectors(rpadiCollectors)

# Transform to data frame with pp detection
detectData <- detectDataFunc(rpadiLists, focalSpecies = "Rhopalosiphum.padi") 
detectData <- dplyr::filter(detectData, !is.na(aet) & !is.na(cwd) & !is.na(tmn) & !is.na(tmx))
detectData$lnlist_length <- log(detectData$list_length)
str(detectData)
table(detectData$detection)
...
# Save detectData
saveRDS(detectData, file = "output/rpadi_detection_dataset.rds")
write.csv(detectData, file = "output/rpadi_detection_dataset.csv", row.names = FALSE)
...
nimbleData <- with(detectData, 
                   list(N = N,
                        ...
                        y = detection,
                        siteID = siteID))
saveRDS(nimbleData, file = "output/rpadi_data_nimble_occupancy.rds")
...
#############################################################################################
#### Occupancy analysis of Rhopalosiphum padi
############################################################################################
my.packages <- c("nimble", "coda", "lattice", "akima")
lapply(my.packages, require, character.only = TRUE)

#### Load data set for model fitting
inputData <- readRDS('output/rpadi_data_nimble_occupancy.rds')
source('R_functions/nimble_definitions.R')
...
save(samplesList, file = 'output/rpadi_MCMC_list.RData')

#### Loading saved MCMC run, sames as list, "samplesList"
load(file = 'output/rpadi_MCMC_list.RData')
